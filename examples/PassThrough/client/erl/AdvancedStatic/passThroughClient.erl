...
passThroughDFE(Size, DataIn) ->
    DataOutDFE = 
        try
            ...
            {Client1, {ok, Maxfile}} = thrift_client:call(Client0, 'PassThrough_init', []),
	    ...
	    StartTime3 = erlang:timestamp(),
            ...
            {Client3, {ok, Address_dataIn}} = thrift_client:call(Client2, malloc_int32_t, [Size]),
            {Client4, {ok, ok}} = thrift_client:call(Client3, send_data_int32_t, [Address_dataIn, DataIn]),
            ...
            Action = #passThrough_actions_t_struct {param_N = Size, instream_x = Address_dataIn, outstream_y = Address_dataOut},
	    {Client6, {ok, Address_action}} = thrift_client:call(Client5, send_PassThrough_actions_t, [Action]),
	    {Client7, {ok, ok}} = thrift_client:call(Client6, 'PassThrough_run', [Engine, Address_action]),
            io:format("Pass through time:\t\t\t\t~ws~n", [timer:now_diff(erlang:timestamp(), StartTime6) / 1000000]),
	    ...
	    io:format("Unloading DFE:\t\t\t\t\t~ws~n", [timer:now_diff(erlang:timestamp(), StartTime7) / 1000000]),
            ...
            {Client9, {ok, DataOut}} = thrift_client:call(Client8, receive_data_int32_t, [Address_dataOut, Size]),
	    ...
	    {Client12, {ok, ok}} = thrift_client:call(Client11, 'PassThrough_free', []),
