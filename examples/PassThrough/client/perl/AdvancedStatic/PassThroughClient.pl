...
sub PassThroughDFE {
    ...
    eval {
        ...
        # Initialize maxfile
        $startTime = time;
        my $max_file = $client->PassThrough_init();
        ...
        # Action default
        $startTime = time;
        my $action = new com::maxeler::PassThrough::PassThrough_actions_t_struct();
        $action->{param_N} = $size;
        $action->{instream_x} = $address_dataIn;
        $action->{outstream_y} = $address_dataOut;
        my $address_action = $client->send_PassThrough_actions_t($action);
