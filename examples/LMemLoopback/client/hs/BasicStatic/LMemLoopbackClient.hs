...
main = do
    ...
    -- Writing to LMem
    startTime <- getCurrentTime
    e <- try (Client.lMemLoopback_writeLMem client (fromIntegral 0) (fromIntegral sizeBytes) (address_inA)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    startTime <- getCurrentTime
    e <- try (Client.lMemLoopback_writeLMem client (fromIntegral sizeBytes) (fromIntegral sizeBytes) (address_inB)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    -- Action default
    startTime <- getCurrentTime
    e <- try (Client.lMemLoopback client (fromIntegral size)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    -- Reading from LMem
    startTime <- getCurrentTime
    e <- try (Client.lMemLoopback_readLMem client (fromIntegral (2 * sizeBytes)) (fromIntegral sizeBytes) (address_dataOut)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    stopTime <- getCurrentTime
