...
// LMemLoopbackDFE Dynamic implementation
func LMemLoopbackDFE(size int64, inA []int32, inB []int32) ([]int32){
    ...
    // Write to LMem
    startTime = time.Now()
    actions, err := client.MaxActionsInit(maxfile, "writeLMem"); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "address", 0); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "nbytes", sizeBytes); checkForErrors(err)
    err = client.MaxQueueInput(actions, "cpu_to_lmem", addressInA, sizeBytes); checkForErrors(err)

    err = client.MaxRun(engine, actions); checkForErrors(err)

    actions, err = client.MaxActionsInit(maxfile, "writeLMem"); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "address", sizeBytes); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "nbytes", sizeBytes); checkForErrors(err)
    err = client.MaxQueueInput(actions, "cpu_to_lmem", addressInB, sizeBytes); checkForErrors(err)
    ...
    // Read from LMem
    startTime = time.Now()
    actions, err = client.MaxActionsInit(maxfile, "readLMem"); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "address", 2 * sizeBytes); checkForErrors(err)
    err = client.MaxSetParamUint64t(actions, "nbytes", sizeBytes); checkForErrors(err)
    err = client.MaxQueueOutput(actions, "lmem_to_cpu", addressOutData, sizeBytes); checkForErrors(err)
