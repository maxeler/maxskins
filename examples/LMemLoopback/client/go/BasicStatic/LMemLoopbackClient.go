...
// LMemLoopbackDFE BasicStatic implementation
func LMemLoopbackDFE(size int64, inA []int32, inB []int32) ([]int32){
    ...
    // Write to LMem
    startTime = time.Now()
    err = client.LMemLoopbackWriteLMem(0, sizeBytes, addressInA); checkForErrors(err)
    err = client.LMemLoopbackWriteLMem(sizeBytes, sizeBytes, addressInB); checkForErrors(err)
    currentTime = calcTime(startTime)
    fmt.Printf("Writing to LMem:\t\t\t\t%.5fs\n", currentTime)

    // Action default
    startTime = time.Now()
    err = client.LMemLoopback(int32(size)); checkForErrors(err)
    currentTime = calcTime(startTime)
    fmt.Printf("LMemLoopback time:\t\t\t\t%.5fs\n", currentTime)

    // Read from LMem
    startTime = time.Now()
    err = client.LMemLoopbackReadLMem(2 * sizeBytes, sizeBytes, addressOutData); checkForErrors(err)
    currentTime = calcTime(startTime)
