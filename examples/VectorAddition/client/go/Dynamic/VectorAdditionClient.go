...
// VectorAdditionDFE Dynamic implementation
func VectorAdditionDFE(x []int32, y []int32, scalar int64, size int64) ([]int32) {
     ...
     // Writing to LMem
     startTime = time.Now()
     act, err := client.MaxActionsInit(maxfile, "writeLMem"); checkForErrors(err)
     err = client.MaxSetParamUint64t(act, "address", 0); checkForErrors(err)
     err = client.MaxSetParamUint64t(act, "nbytes", size*4); checkForErrors(err)
     err = client.MaxQueueInput(act, "cpu_to_lmem", addressX, size * 4); checkForErrors(err)
     err = client.MaxRun(engine, act); checkForErrors(err)
     ...
     // Action default
     startTime = time.Now()
     act, err = client.MaxActionsInit(maxfile, "default"); checkForErrors(err)
     err = client.MaxSetParamUint64t(act, "N", size); checkForErrors(err)
     err = client.MaxSetParamUint64t(act, "A", scalar); checkForErrors(err)
     err = client.MaxQueueInput(act, "y", addressY, size * 4); checkForErrors(err)
     err = client.MaxQueueOutput(act, "s", addressS, size * 4); checkForErrors(err)
     err = client.MaxRun(engine, act); checkForErrors(err)
     //err = client.Free(act); checkForErrors(err)
     currentTime = calcTime(startTime)
