...
main = do
    ...
    -- Writing to LMem
    startTime <- getCurrentTime
    let actions_lmem = VectorAddition_writeLMem_actions_t_struct (Just (fromIntegral 0)) (Just (fromIntegral size * 4)) (Just (fromIntegral address_x))

    e <- try (Client.send_VectorAddition_writeLMem_actions_t client actions_lmem) :: IO (Either SomeException Int64)
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let address_actions_lmem = getRight e

    e <- try (Client.vectorAddition_writeLMem_run client engine address_actions_lmem) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    -- Action default
    startTime <- getCurrentTime
    let action = VectorAddition_actions_t_struct (Just (fromIntegral scalar)) (Just (fromIntegral size)) (Just (fromIntegral address_y)) (Just (fromIntegral address_dataOut))

    e <- try (Client.send_VectorAddition_actions_t client action) :: IO (Either SomeException Int64)
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let address_action = getRight e

    e <- try (Client.vectorAddition_run client engine address_action) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
