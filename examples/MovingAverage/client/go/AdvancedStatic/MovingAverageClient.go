...
func main() {
    ...
    addressAction, err := client.Send_MovingAverageActionsT(action); checkForErrors(err)
    err = client.MovingAverageRun(engine, addressAction); checkForErrors(err)
    currentTime = calcTime(startTime)
    ...
    err = client.Free(addressAction); checkForErrors(err)
    currentTime = calcTime(startTime)
