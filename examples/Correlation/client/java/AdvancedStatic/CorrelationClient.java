...
import com.maxeler.correlation.correlation_actions_t_struct;
import com.maxeler.correlation.correlation_loadLMem_actions_t_struct;
...
/** Correlation AdvancedStatic example. */
public final class CorrelationClient {
  ...
  private static final int  SIZE_OF_DOUBLE = 64;
  /** Size of int in bits. */
  ...
  public static double[] correlateDfe(
    ...
    try {
      ...
      correlation_loadLMem_actions_t_struct actionsLoadLMem =
          new correlation_loadLMem_actions_t_struct();
      ...
      final long addressActionsLoadLMem =
          client.send_correlation_loadLMem_actions_t(actionsLoadLMem);
      ...
      // scalar input
      actions.setParam_numBursts(numBursts);
      // scalar input
      actions.setParam_numSteps(numTimesteps);
      // scalar input
      actions.setParam_numVariables(numTimeseries);
      // scalar input
      actions.setParam_outputLastStep(1);
      // scalar input
      actions.setParam_windowSize(windowSize);
      // streaming reordered input
      actions.setInstream_in_precalculations(addressPrecalculations);
      // streaming reordered input
      actions.setInstream_in_variable_pair(addressDataPairs);
      // streaming reordered input
      actions.setOutstream_out_correlation(addressOutCorrelation);
      // streaming reordered input
      actions.setOutstream_out_indices(addressOutIndices);
