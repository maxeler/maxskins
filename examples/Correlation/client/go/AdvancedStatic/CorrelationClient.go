...
func CorrelateDFE (data [][]float64, sizeTimeseries int64, numTimeseries int64) ([]float64) {
    ...
    actionsLoadLMem := correlation.NewCorrelationLoadLMemActionsTStruct()
    actionsLoadLMem.ParamNumBursts = numBursts
    actionsLoadLMem.Param_CorrelationKernelLoopLength = addressLoopLength
    actionsLoadLMem.InstreamInMemLoad = addressInMemLoad

    addressActionsLoadLMem, err := client.SendCorrelationLoadLMemActionsT(actionsLoadLMem); checkForErrors(err)
    err = client.CorrelationLoadLMemRun(engine, addressActionsLoadLMem); checkForErrors(err)
    ...
    action := correlation.NewCorrelationActionsTStruct()
    action.ParamNumBursts = numBursts				       // scalar input
    action.ParamNumSteps = numTimesteps				       // scalar input
    action.ParamNumVariables = numTimeseries			       // scalar input
    action.ParamOutputLastStep = 1				       // scalar input
    action.ParamWindowSize = windowSize				       // scalar input
    action.InstreamInPrecalculations = addressPrecalculations	       // streaming reordered input
    action.InstreamInVariablePair = addressDataPairs		       // streaming reordered input
    action.OutstreamOutCorrelation = addressOutCorrelation	       // streaming unordered output
    action.OutstreamOutIndices = addressOutIndices		       // streaming unordered output

    addressActions, err := client.SendCorrelationActionsT(action); checkForErrors(err)
    err = client.CorrelationRun(engine, addressActions); checkForErrors(err)
    ...
    err = client.Free(addressActionsLoadLMem); checkForErrors(err)
    err = client.Free(addressActions); checkForErrors(err)
    currentTime = calcTime(startTime)
