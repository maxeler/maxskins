...
using ::com::maxeler::correlation::correlation_actions_t_struct;
using ::com::maxeler::correlation::correlation_loadLMem_actions_t_struct;
...
void CorrelateDFE(double** data, uint64_t sizeTimeseries,
  ...
  try {
    ...
    actionsLoadLMem.__set_param_CorrelationKernel_loopLength(
        addressLoopLength);
    actionsLoadLMem.__set_instream_in_memLoad(addressInMemLoad);
    remote_ptr addressActionsLoadLMem =
    ...
    // scalar input
    actions.__set_param_numBursts(numBursts);
    // scalar input
    actions.__set_param_numSteps(numTimesteps);
    // scalar input
    actions.__set_param_numVariables(numTimeseries);
    // scalar input
    actions.__set_param_outputLastStep(1);
    // scalar input
    actions.__set_param_windowSize(windowSize);
    // streaming reordered input
    actions.__set_instream_in_precalculations(addressPrecalculations);
    // streaming reordered input
    actions.__set_instream_in_variable_pair(addressDataPairs);
    // streaming unordered output
    actions.__set_outstream_out_correlation(addressOutCorrelation);
    // streaming unordered output
    actions.__set_outstream_out_indices(addressOutIndices);
