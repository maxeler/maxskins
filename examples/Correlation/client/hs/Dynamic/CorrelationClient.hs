...
correlateDFE dataIn numTimeseries sizeTimeseries    =    do
    ...
    -- Initialize maxfile
    startTime <- getCurrentTime
    e <- try (Client.correlation_init client) :: IO (Either SomeException Int64)
    case e of
    ...
    e <- try (Client.max_actions_init client maxfile (pack "loadLMem")) :: IO (Either SomeException Int64)
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let actions = getRight e

    e <- try (Client.max_set_param_uint64t client actions (pack "numBursts") (fromIntegral numBursts)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_queue_input client actions (pack "in_memLoad") (fromIntegral addressInMemLoad) (fromIntegral (numBursts * burstSize))) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_ignore_scalar client actions (pack "LMemCommandsKernel") (pack "run_cycle_count")) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_ignore_scalar client actions (pack "CorrelationKernel") (pack "run_cycle_count")) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let actionsDefault = getRight e

    e <- try (Client.max_set_param_uint64t client actionsDefault (pack "numBursts") (fromIntegral numBursts)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_set_param_uint64t client actionsDefault (pack "numSteps") (fromIntegral numTimesteps)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_set_param_uint64t client actionsDefault (pack "numVariables") (fromIntegral numTimeseries)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_set_param_uint64t client actionsDefault (pack "outputLastStep") (fromIntegral 1)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_set_param_double client actionsDefault (pack "windowSize") (fromIntegral windowSize)) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_queue_input client actionsDefault (pack "in_precalculations") (fromIntegral addressPrecalculations) (fromIntegral (precalculationsSize * sizeOfDoubleInBytes))) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_queue_input client actionsDefault (pack "in_variable_pair") (fromIntegral addressDataPairs) (fromIntegral (dataPairsSize * sizeOfDoubleInBytes))) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    e <- try (Client.max_queue_output client actionsDefault (pack "out_correlation") (fromIntegral addressOutCorrelation) (fromIntegral (outCorrelationSize * sizeOfDoubleInBytes))) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    e <- try (Client.max_queue_output client actionsDefault (pack "out_indices") (fromIntegral addressOutIndices) (fromIntegral (outIndicesSize * sizeOfIntInBytes))) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()

    e <- try (Client.max_run client engine actionsDefault) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
