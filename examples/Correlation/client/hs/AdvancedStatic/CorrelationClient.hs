...
correlateDFE dataIn numTimeseries sizeTimeseries    =    do
    ...
    let actionsLoadLMem =  Correlation_loadLMem_actions_t_struct (Just (fromIntegral numBursts)) (Just addressLoopLength) (Just addressInMemLoad)

    e <- try (Client.send_correlation_loadLMem_actions_t client actionsLoadLMem) :: IO (Either SomeException (Int64))
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let addressActionsLoadLMem = getRight e
    stopTime <- getCurrentTime

    e <- try (Client.correlation_loadLMem_run client engine addressActionsLoadLMem) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    ...
    let action = Correlation_actions_t_struct (Just (fromIntegral numBursts)) (Just(fromIntegral numTimesteps)) (Just(fromIntegral numTimeseries)) (Just 1) (Just (fromIntegral windowSize)) (Just addressPrecalculations) (Just addressDataPairs) (Just addressOutCorrelation) (Just addressOutIndices)

    e <- try (Client.send_correlation_actions_t client action) :: IO (Either SomeException Int64)
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let address_action = getRight e   

    e <- try (Client.correlation_run client engine address_action) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let address_action = getRight e
    ...
    case e of
        ...
        Right ex -> return ()
    stopTime <- getCurrentTime
    e <- try (Client.free client addressActionsLoadLMem) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    stopTime <- getCurrentTime
