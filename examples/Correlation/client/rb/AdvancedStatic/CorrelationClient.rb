...
def correlate_dfe(data, size_timeseries, num_timeseries, correlations)
  ...
  # Initialize maxfile
  start_time = Time.now
  maxfile = client.correlation_init
  current_time = (Time.now - start_time).round(5)
  ...
  actions_load_lmem = Correlation_loadLMem_actions_t_struct.new
  actions_load_lmem.param_numBursts = num_bursts
  actions_load_lmem.param_CorrelationKernel_loopLength = address_loop_length
  actions_load_lmem.instream_in_memLoad = address_in_mem_load
  ...
  actions = Correlation_actions_t_struct.new
  actions.param_numBursts = num_bursts        # scalar input
  actions.param_numSteps = num_timesteps      # scalar input
  actions.param_numVariables = num_timeseries # scalar input
  actions.param_outputLastStep = 1            # scalar input
  actions.param_windowSize = window_size      # scalar input
  actions.instream_in_precalculations =
    address_precalculations                   # streaming reordered input
  actions.instream_in_variable_pair =
    address_data_pairs                        # streaming reordered input
  actions.outstream_out_correlation =
    address_out_correlation                   # streaming reordered input
  actions.outstream_out_indices =
    address_out_indices                       # streaming reordered input
