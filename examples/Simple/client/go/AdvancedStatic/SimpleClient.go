...
// SimpleDFE AdvancedStatic implementation
func SimpleDFE(size int64, dataIn []float64) ([]float64) {
    ...
    // Action default
    startTime = time.Now()
    action := Simple.NewSimpleActionsTStruct()
    action.Param_N = int32(size)
    action.InstreamX = addressDataIn
    action.OutstreamY = addressDataOut
    addressAction, err := client.Send_SimpleActionsT(action); checkForErrors(err)
    err = client.SimpleRun(engine, addressAction); checkForErrors(err)
    currentTime = calcTime(startTime)
