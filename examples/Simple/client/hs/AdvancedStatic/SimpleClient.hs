...
main = do
    ...
    let action = Simple_actions_t_struct (Just (fromIntegral size)) (Just (fromIntegral address_dataIn)) (Just (fromIntegral address_dataOut))

    e <- try (Client.send_Simple_actions_t client action) :: IO (Either SomeException Int64)
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
    let address_action = getRight e

    e <- try (Client.simple_run client engine address_action) :: IO (Either SomeException ())
    case e of
        Left ex -> putStrLn $ "Caught exception: " Data.List.++ show ex
        Right ex -> return ()
